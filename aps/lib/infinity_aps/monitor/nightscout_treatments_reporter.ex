defmodule InfinityAPS.Monitor.NightscoutTreatmentsReporter do
  ...
  defp filter_history({entry, _})
       when entry in [:bolus_normal, :bolus_wizard_estimate, :alarm_sensor, :cal_bg_for_ph],
       do: true
  ...
  defp map_history(
         {:alarm_sensor, %{amount: amount, alarm_type: alarm_type, timestamp: timestamp}},
         local_timezone
       ) do
    %{
      eventType: "Note",
  ...
  defp map_history(
         {:bolus_wizard_group,
          %{
            ...
            bolus_normal: %{amount: amount, timestamp: timestamp}
          }},
         local_timezone
       ) do
    %{
      ...
      insulin: amount,
      glucose: bg,
      glucoseType: "BolusWizard"
    }
  end

  # bolus with no bolus wizard
  defp map_history(
         {:bolus_wizard_group, %{bolus_normal: %{amount: amount, timestamp: timestamp}}},
         local_timezone
       ) do
    %{
  ...
  defp map_history(
         {:bolus_wizard_estimate, %{carbohydrates: carbohydrates, timestamp: timestamp, bg: bg}},
         local_timezone
       ) do
    %{
  ...
  defp map_history(
         {:temp_basal_group, %{temp_basal_duration: temp_basal_duration, temp_basal: temp_basal}},
         local_timezone
       ) do
